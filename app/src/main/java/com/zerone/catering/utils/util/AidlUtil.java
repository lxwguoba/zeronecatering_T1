package com.zerone.catering.utils.util;
...
//                woyouService.printColumnsText(tableItem.getText(), tableItem.getWidth(), tableItem.getAlign(), null);
    ...
    /**
     * 打印表格 可以根据width自动分配没一列所占的长度大小
     * @param list  展示的内容
     * @param fontsize  字体大小
     *@param    isBold   字体是否加粗
     */
    public void printTable(LinkedList<TableItem> list,int fontsize,boolean isBold) {
        if (woyouService == null) {
            Toast.makeText(context,R.string.toast_2,Toast.LENGTH_LONG).show();
            return;
        }
    ...
    * 空打三行！
    ...
    public  void  printbaidu(){
        byte[]  rv = BytesUtil.getBaiduTestBytes();
        try {
            woyouService.sendRAWData(rv,null);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
